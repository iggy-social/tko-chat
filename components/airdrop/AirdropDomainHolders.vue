<template>
	<div>
		<p class="text-center">
			Claim {{ domainChatReward }} {{ $config.chatTokenSymbol }} airdrop for each {{ $config.tldName }} domain
			that you hold.
		</p>
		...
		<p class="text-center">
			<small>
				Note that someone else can claim tokens (for a given domain) for you and save you on gas fees. You will
				still receive the same amount of {{ $config.chatTokenSymbol }} tokens as if you've made the claim
				yourself.
			</small>
		</p>
	</div>
		...
		async claim() {
			...
			try {
				...
				if (receipt.status === 1) {
					...
					this.toast(
						'Airdrop for ' + cleanDomainName + this.$config.tldName + ' has been successfully claimed!',
						{
							type: 'success',
							onClick: () =>
								window.open(this.$config.blockExplorerBaseUrl + '/tx/' + tx.hash, '_blank').focus(),
