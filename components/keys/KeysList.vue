		...
		<table class="table bg-transparent text-center">
			...
			<tbody>
				<tr v-for="domain in domainObjects" :key="domain.name">
					<td class="bg-transparent text-break">
						<NuxtLink :to="'/keys?username=' + domain.name"
							>{{ domain.name }}{{ $config.tldName }}</NuxtLink
						>
					</td>
...
export default {
	...
	methods: {
		...
		async getKeysPrices() {
			...
			const keysContract = new ethers.Contract(this.$config.keysAddress, keysInterface, provider)
