<template>
	<div>
		...
		<p v-if="!isUserEligible" class="text-center">Not eligible...</p>
		...
		<div v-if="isUserEligible">
			...
			<div class="d-flex justify-content-center mt-4 mb-4">
				<button :disabled="waiting" @click="claim" class="btn btn-outline-primary">
					<span
						v-if="waiting"
...
export default {
	...
	methods: {
		checkEligibility() {
			const eligibility = this.claimersData.claimers.findIndex(
				item => item[0].toLowerCase() === this.address.toLowerCase(),
			)
