<template>
	<span
		@click="fetchMintData"
		class="cursor-pointer ms-3 hover-color"
		data-bs-toggle="modal"
		:data-bs-target="'#mintPostModal' + post.stream_id"
	>
	...
	<div
		class="modal fade"
		:id="'mintPostModal' + post.stream_id"
		tabindex="-1"
		aria-labelledby="mintPostModalLabel"
		aria-hidden="true"
	>
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<button
						type="button"
						:id="'closeMintPostModal' + post.stream_id"
						class="btn-close"
				...
				<div class="modal-body">
					...
					<p>Minting price: {{ postPrice }} {{ $config.tokenSymbol }}</p>

					<div class="form-check">
						<input class="form-check-input" type="checkbox" id="flexCheckChecked" v-model="makePost" />

						<label class="form-check-label" for="flexCheckChecked"> Make a post about the minting </label>
					</div>
				</div>
				<div class="modal-footer">
					<button
						...
						@click="mintPost"
						:disabled="!isActivated || waitingMint"
					>
						<span
							v-if="waitingMint"
	...
	methods: {
		...
		async fetchMintData() {
			if (this.isActivated) {
				...
				const postPriceWei = await iggyContract.getPostPrice(
					this.post.stream_id,
		...
		async mintPost() {
			if (this.isActivated) {
				...
				const iggyMinterContract = new ethers.Contract(
					this.$config.iggyPostMinterAddress,
					iggyPostMinterInterface,
					this.signer,
				)
				...
				try {
					...
					if (receipt.status === 1) {
						...
						if (this.makePost && this.userStore.getIsConnectedToOrbis) {
							...
							const iggyStatsContract = new ethers.Contract(
								this.$config.iggyPostStatsAddress,
								iggyStatsInterface,
								this.signer,
							)
