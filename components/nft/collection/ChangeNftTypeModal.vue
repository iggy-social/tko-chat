<template>
	<div
		class="modal fade"
		id="changeNftTypeModal"
		tabindex="-1"
		aria-labelledby="changeNftTypeModalLabel"
		aria-hidden="true"
	>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="dropdown">
						...
						<div class="dropdown-menu text-break">
							<span
								v-for="(option, index) in editImageOptions"
								:key="index"
					...
					<div class="mt-3">
						<p v-if="typeChoice == 0">
							The "Onchain image(s)" option means that metadata is stored onchain (within the smart
							contract itself). Use this option if you want to add an image URL in the field below (not a
							metadata file). This is the default option for NFTs created through this launchpad.
						</p>

						<p v-if="typeChoice == 1">
							The "Static metadata URL" option means that all the NFTs in the collection have the same
							(static) image and meta data. The metadata is stored offchain, usually on IPFS or on a
							centralized server. Use this option if you have a URL of a single metadata
							<strong>file</strong> (all NFTs have the same metadata and image).
						</p>

						<p v-if="typeChoice == 2">
							The "Generative metadata (.json)" option means that all the NFTs in the collection have a
							different image and different meta data. The metadata is stored offchain, usually on IPFS or
							on a centralized server. Use this option if you have a metadata <strong>folder</strong> URL
							with multiple metadata files in it, where these files HAVE a .json extension.
						</p>

						<p v-if="typeChoice == 3">
							The "Generative metadata" option means that all the NFTs in the collection have a different
							image and different meta data. The metadata is stored offchain, usually on IPFS or on a
							centralized server. Use this option if you have a metadata <strong>folder</strong> URL with
							multiple metadata files in it, where these files DO NOT HAVE a .json extension.
						</p>
					</div>
					...
					<div class="mt-4" v-if="typeChoice > 0">
						<label for="collectionPreviewUrl" class="form-label">
							<strong> Change collection preview image (optional): </strong>
						</label>

						<input
							v-model="editImagePreviewUrl"
							type="text"
							class="form-control"
							id="collectionPreviewUrl"
							aria-describedby="previewImageHelp"
						/>
						<div id="previewImageHelp" class="form-text">
							This is a preview image for the whole collection, not for individual NFTs.
						</div>
					</div>
				</div>
				...
				<div class="modal-footer">
					...
					<button
						...
						:disabled="!editImageMetadataUrl || waitingMetadata"
					>
						<span
							v-if="waitingMetadata"
