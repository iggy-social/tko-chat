	...
	<div>
		<div class="mb-3">
			...
			<div class="form-text" id="basic-addon4" v-if="hasBlankCharacter">
				<i class="bi bi-exclamation-triangle-fill"></i>
				This domain name contains a blank character: {{ encodeURIComponent(inputReceiver) }}. Proceed with
				caution.
			</div>
		</div>
			...
			if (this.inputReceiver.includes('.')) {
				if (!this.inputReceiver.endsWith(this.$config.tldName)) {
					return this.toast.error(
						'Invalid domain name. Only ' + this.$config.tldName + ' domains are supported.',
					)
				}
			}
