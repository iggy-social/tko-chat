<template>
	<div>
		<p class="text-center">
			Unstake {{ $config.stakeTokenSymbol }} to receive back {{ $config.lpTokenSymbol }} (it will also auto-claim
	...
	methods: {
		async withdrawal() {
			...
			try {
				...
				if (receipt.status === 1) {
					this.userStore.setLpTokenBalanceWei(
						this.userStore.getLpTokenBalanceWei.add(this.withdrawalAmountWei),
					)
					this.userStore.setStakeTokenBalanceWei(
						this.userStore.getStakeTokenBalanceWei.sub(this.withdrawalAmountWei),
					)
