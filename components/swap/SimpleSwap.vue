<template>
	<div>
		...
		<div class="input-group mb-1">
			...
			<ul class="dropdown-menu p-2">
				...
				<li v-for="token in filteredTokensInput" :key="token.address" class="cursor-pointer">
					<span @click="selectInputToken(token)" class="dropdown-item"
						>{{ token.symbol }} ({{ token.name }})</span
					>
				</li>
			</ul>
		...
		<!-- Arrow down -->
		<h4 @click="getOutputAmount" class="text-center mt-2 cursor-pointer">
		...
		<div class="input-group mt-4">
			...
			<ul class="dropdown-menu p-2">
				<input class="form-control mb-2" placeholder="Filter tokens" v-model="filterTextOutput" />

				<li v-for="token in filteredTokensOutput" :key="token.address" class="cursor-pointer">
					<span @click="selectOutputToken(token)" class="dropdown-item"
						>{{ token.symbol }} ({{ token.name }})</span
					>
				</li>
			</ul>

			<input type="text" class="form-control" :value="outputTokenAmount" :placeholder="outputText" disabled />
      ...
      -->
		...
		<small v-if="outputTokenAmount && !bothTokensAreNativeCoinOrWrappedTokenOrSame">
			<em>
				You will get at least {{ outputTokenAmount }} {{ outputToken.symbol }}, but probably more ({{
					siteStore.getSlippage
				}}% slippage).
			</em>
		</small>
		...
		<div class="d-flex justify-content-center mt-4">
			...
			<button
				v-if="
					...
					!priceImpactTooHigh &&
					!priceImpactTooHigh &&
					(!allowanceTooLow || unwrappingWrappedNativeCoin)
				"
				:disabled="
					!inputToken ||
					!outputToken ||
			...
			<button
				v-if="
					isActivated && isSupportedChain && inputTokenAmount && bothTokensAreTheSame && !priceImpactTooHigh
				"
				:disabled="true"
				class="btn btn-outline-primary"
				type="button"
      ...
      */
			...
			if (this.inputTokenAmount) {
				...
				if (this.bothTokensAreNativeCoinOrWrappedTokenOrSame) {
					...
					const outputWei = await getOutputTokenAmount(
						this.signer,
						this.inputToken,
						this.outputToken,
				...
				this.priceImpact = await getPriceImpactBps(
					this.signer,
					this.inputToken,
					this.outputToken,
					this.inputTokenAmount,
		...
		async selectInputToken(token) {
			...
			if (token.address == ethers.constants.AddressZero) {
				...
				this.inputTokenAllowance = await this.getTokenAllowance(
					token,
					this.address,
					this.routerAddress,
					this.signer,
