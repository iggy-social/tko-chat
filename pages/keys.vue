<template>
			...
			<div class="d-flex justify-content-center">
				<div class="col-12 col-lg-8">
					<p class="text-break text-center mt-3 mb-4">
						Search for a domain and get its Friend Key to unlock a chat room with the domain holder and
						other key holders.
					</p>
					...
					<!-- Get Data button -->
					<div
						v-if="isActivated && isSupportedChain && !showBuySellButtons"
					...
					<!-- Buy & Sell buttons -->
					<div
						v-if="isActivated && isSupportedChain && showBuySellButtons && !domainNotExists"
	...
	<!-- KeysList -->
	<div
		v-if="!domainNotExists && !showBuySellButtons && !buyKeyToChat && !isKeyHolder"
		class="card border mt-3 scroll-500"
	>
		<div class="card-body">
	...
	<div v-if="domainNotExists && !showBuySellButtons" class="card border mt-3 scroll-500">
		<div class="card-body">
			...
			<div class="d-flex justify-content-center">
				<div class="col-12 col-lg-8">
					<div class="text-break mt-3 mb-4">
	...
	<div v-if="buyKeyToChat" class="card border mt-3 scroll-500">
		<div class="card-body">
			...
			<div class="d-flex justify-content-center">
				<div class="col-12 col-lg-8">
					<p class="text-break text-center mt-3 mb-4">
						The chat is open only for key holders. Buy a key to see the chat and talk with the domain holder
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h1 class="modal-title fs-5" id="buyKeyModalLabel">
						Buy one {{ cleanDomainName }}{{ $config.tldName }} key
					</h1>
					<button
						id="closeBuyKeyModal"
						type="button"
				...
				<div class="modal-body">
					Buy 1 {{ cleanDomainName }}{{ $config.tldName }} key for {{ buyKeyPrice }}
					{{ $config.tokenSymbol }}?
				</div>
	...
	<div class="modal fade" id="sellKeyModal" tabindex="-1" aria-labelledby="sellKeyModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h1 class="modal-title fs-5" id="sellKeyModalLabel">
						Sell one {{ cleanDomainName }}{{ $config.tldName }} key
					</h1>
					<button
						id="closeSellKeyModal"
						type="button"
						class="btn-close"
						data-bs-dismiss="modal"
						aria-label="Close"
				...
				<div class="modal-body">
					Sell 1 {{ cleanDomainName }}{{ $config.tldName }} key for {{ sellKeyPrice }}
					{{ $config.tokenSymbol }}?
				</div>
...
export default {
	...
	methods: {
		async buyKey() {
			...
			const keysContract = new ethers.Contract(this.$config.keysAddress, keysInterface, this.signer)
				...
				if (receipt.status === 1) {
					...
					this.toast(
						this.cleanDomainName +
							this.$config.tldName +
							' friend key has been successfully claimed! Now you can chat.',
						{
							type: 'success',
							onClick: () =>
								window.open(this.$config.blockExplorerBaseUrl + '/tx/' + tx.hash, '_blank').focus(),
		...
		async sellKey() {
			...
			try {
				...
				if (receipt.status === 1) {
					...
					this.toast(
						this.cleanDomainName + this.$config.tldName + ' friend key has been successfully sold!',
						{
							type: 'success',
							onClick: () =>
								window.open(this.$config.blockExplorerBaseUrl + '/tx/' + tx.hash, '_blank').focus(),
