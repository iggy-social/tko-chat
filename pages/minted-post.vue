	...
	<div>
		<Head>
			...
			<Meta
				...
				:content="'Check out this minted post NFT on ' + $config.projectName + '!'"
			/>
				...
				<ul class="text-break">
					...
					<li>
						<a
							...
							target="_blank"
							:href="$config.marketplacePostNftItemUrl + getQueryId"
						>
					...
					<li>
						<a
							class="text-decoration-none"
							target="_blank"
							:href="$config.blockExplorerBaseUrl + '/token/' + $config.iggyPostAddress"
						>
...
export default {
	...
	methods: {
		async getPostObject() {
			...
			const iggyPostContract = new ethers.Contract(this.$config.iggyPostAddress, iggyPostInterface, provider)
