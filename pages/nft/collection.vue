						...
						<ul class="dropdown-menu">
							...
							<li v-if="isCurrentAddressOwner">
								<span
									class="dropdown-item cursor-pointer"
									data-bs-toggle="modal"
									data-bs-target="#changeNftTypeModal"
								>
							...
							<li>
								<span class="dropdown-item cursor-pointer" @click="getCollectionDetails(true)"
			...
			<div class="d-flex justify-content-center">
				<div class="col-12 col-lg-8">
					<p class="text-break text-center mt-3 mb-4">
						This NFT's chat is open only for NFT holders. Buy an NFT to see the chat and talk with the NFT
						creator and other NFT holders.
					</p>
				</div>
			</div>
		</div>
	...
	watch: {
		...
		cAddress(newValue, oldValue) {
			...
			if (
				...
				newValue !== oldValue &&
				this.cAddress &&
				this.$route.path === '/nft/collection'
			) {
				// check if address is valid
				if (!ethers.utils.isAddress(this.cAddress)) {
					this.toast('Invalid NFT address.', { type: 'error' })
