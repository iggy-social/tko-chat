<template>
	<Head>
		...
		<Meta
			...
			:content="'Create your very own NFT collection on ' + $config.projectName + '!'"
		/>
		...
		<div class="card-body">
			...
			<div class="mb-2">
				...
				<div id="cImageHelp" class="form-text">
					Even if you want a generative PFP collection, put a single preview image for now and you will change
					it to a metadata link later.
				</div>
			</div>
			...
			<div class="mb-4">
				...
				<div id="uniqueIdHelp" class="form-text">
					This is just in case the frontend will not show you the NFT collection address and you'll need to
					find it manually.
				</div>
			</div>
			...
			<div class="mb-4">
				...
				<div id="ratioHelp" class="form-text">
					...
					for mint #5 will be {{ getLessDecimals(calculatePrice(5, ratio)) }} {{ $config.tokenSymbol }}, for
					mint #15 will be {{ getLessDecimals(calculatePrice(15, ratio)) }} {{ $config.tokenSymbol }}, for
					mint #30 will be {{ getLessDecimals(calculatePrice(30, ratio)) }} {{ $config.tokenSymbol }}, etc.
				</div>
			</div>
...
export default {
	...
	methods: {
		calculatePrice(nftId, ratio) {
			...
			return (
				((((supply * (supply + 1) * (2 * supply + 1) - (supply - 1) * supply * (2 * (supply - 1) + 1)) *
					10000) /
					42069) *
					Number(ratio)) /
				31337
		...
		async makeOrbisPost(nftContractAddress) {
			if (this.userStore.getIsConnectedToOrbis) {
				try {
					...
					const options = {
						body:
							'I have launched a new NFT collection: ' + this.cName + ' <br /><br />Check it out here 👇',
